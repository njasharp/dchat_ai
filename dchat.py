...
# Streamlit UI layout
st.set_page_config(layout="wide", page_title="Enhanced advance reasoning support")
st.image("p1.png", width=360)
st.sidebar.image("p2.png", width=260)

# Initialize session state
if "conversations" not in st.session_state:
    st.session_state.conversations = {"2024-04-15 15:27:16": {"summary": "", "details": "", "system_prompt": "You are a helpful assistant that provides summaries and details based on user queries and given context with advance reasoning support."}}
...
with st.sidebar:
    ...
    with st.expander("Advanced Settings"):
        ...
        st.session_state.max_length = st.slider("Max Response Length", 100, 2000, 1000)
    ...
    if st.button("New Conversation"):
        ...
        st.session_state.conversations[new_conv] = {"summary": "", "details": "", "system_prompt": "You are a helpful assistant that provides summaries and details based on user queries and given context with advance reasoning support."}
    ...
    new_name = st.text_area("Rename", value=selected_conversation)
    ...
    selected_file = st.selectbox("Select a file", list(st.session_state.files.keys()) if st.session_state.files else ["No files uploaded"])
    if st.button("Generate Report") and selected_file != "No files uploaded":
        ...
        if reasoning_type == "Advance Steps":
            ...
            with st.spinner("Generating report..."):
                report_summary, report_details = search_and_summarize(
                    f"Generate a detailed report for the file: {selected_file}", 
                    model_id, 
                    system_prompt, 
                    context, 
                    reasoning_type, 
...
with col1:
    ...
    user_input = st.text_area("Enter your query here...", key="query_input")
    if st.button("Send"):
        if user_input:
            context = st.session_state.files.get(selected_file, "") if selected_file != "No files uploaded" else ""
            model_id = SUPPORTED_MODELS[st.session_state.selected_model]
            reasoning_type = st.session_state.reasoning_type
            selected_task = st.session_state.selected_task if reasoning_type == "Multi-path" else None
            system_prompt = st.session_state.conversations[st.session_state.active_conversation].get("system_prompt", "")
    ...
    st.session_state.conversations[st.session_state.active_conversation]["system_prompt"] = st.text_area(
        "System Prompt", 
        value=st.session_state.conversations[st.session_state.active_conversation].get("system_prompt", ""), 
        height=100, 
...
with col2:
    ...
    st.text_area(
        "Details", 
        value=st.session_state.conversations[st.session_state.active_conversation].get("details", ""), 
        height=600, 
        key="details_area"
    )
...
st.info("deepchat - advance reasoning support build by dw 2-2-25")